...
if (isset($_POST['simpan'])) {
    ...
    // Validasi data
    if ($id_proker <= 0 || $pelaksanaan_ke <= 0 || empty($tanggal_pelaksanaan) || $jumlah_peserta < 0 || empty($dampak_ke_himpunan)) {
        ...
        $query = "INSERT INTO pelaksanaan_proker (id_proker, pelaksanaan_ke, tanggal_pelaksanaan, jumlah_peserta, dampak_ke_himpunan) 
                  VALUES ('$id_proker', '$pelaksanaan_ke', '$tanggal_pelaksanaan', '$jumlah_peserta', '$dampak_ke_himpunan')";

        if (mysqli_query($conn, $query)) {
            // Catat log
            $admin_user = mysqli_real_escape_string($conn, $_SESSION['username']);
            $log_aksi = mysqli_real_escape_string($conn, "Mencatat pelaksanaan ke-" . $pelaksanaan_ke . " untuk proker " . $nama_proker . " dengan " . $jumlah_peserta . " peserta");
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Add focus styles for textarea dynamically to match input style
            const textarea = document.getElementById('dampak_ke_himpunan');
            if (textarea) {
